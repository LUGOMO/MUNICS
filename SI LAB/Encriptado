...
if len(rutaUsuario) == 1:
    ...
    # print(f"\nResultado inicial del destinatario (fuera del bucle): {final_result.hex()}")
    ...
    for i in range(len(rutaUsuario) - 2, -1, -1):  # Comenzamos desde el penúltimo salto hacia el primero
        ...
        # print(f"\nGenerando nueva clave simétrica para el salto '{hop_actual}': {new_key.hex()}")
        ...
        # print(f"Resultado cifrado con la nueva clave simétrica para el salto '{hop_actual}'.")

        # Obtener la clave pública del **salto actual (hop_actual)**, que es quien debe desencriptar este salto
        clave_publica_hop_str = publicKeysOrder[i]  # Tomamos la clave pública del salto actual
        # print(f"Clave pública del salto actual '{hop_actual}': {clave_publica_hop_str}")
        ...
        #print(f"Clave simétrica cifrada con la clave pública del salto '{hop_actual}'.")
        ...
        if i > 0:
            ...
            # print(f"Concatenación final para el salto '{hop_actual}' realizada. Tamaño del resultado: {len(final_result)} bytes")
        else:
            # En la última iteración (primer salto en la ruta), no concatenamos el ID
            final_result = encrypted_key + final_result
            # print(f"Concatenación sin ID para el último salto (primer salto en ruta). Tamaño del resultado: {len(final_result)} bytes")
...
# print(f"Tamaño final del mensaje cifrado: {len(final_result)} bytes")
