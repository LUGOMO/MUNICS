...
// Modified from OpenZeppelin Contracts (last updated v5.0.0) (access/AccessControl.sol)
...
abstract contract AccessControl is Context, IAccessControl, ERC165 {
    ...
    modifier onlyRoles(bytes32 role1, bytes32 role2) {
        _checkRoles(role1, role2, _msgSender());
        _;
    }

    function _checkRoles(bytes32 role1, bytes32 role2, address account) private view {
        if (!hasRole(role1, account) && !hasRole(role2, account)) {
            revert AccessControlUnauthorizedAccount(account, role2);
        }
    }
     ...
     */
    function getRoleAdmin(bytes32 role) internal view virtual returns (bytes32) {
